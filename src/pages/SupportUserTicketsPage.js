...
const userTickets = [
  { id: 'TCKT1001', name: 'Ali Khan', priority: 'High', status: 'Open', age: '2 hrs', booking: 'BKG12345', message: 'Booking not confirmed after payment.' },
  { id: 'TCKT1003', name: 'Ali Khan', priority: 'Low', status: 'In Progress', age: '3 hrs', booking: 'BKG12345', message: 'Unable to apply coupon.' },
];
...
const SupportUserTicketsPage = () => {
  ...
  const filtered = useMemo(() => userTickets.filter(t => {
    const q = query.toLowerCase();
    return !q || t.id.toLowerCase().includes(q) || t.name.toLowerCase().includes(q) || t.booking.toLowerCase().includes(q) || t.message.toLowerCase().includes(q);
  }), [query]);
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filtered.map(t => (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-sm text-gray-800">{t.name}</td>
                  ...
                  <td className="px-5 py-3 text-right">
                    <button onClick={() => navigate(`/support/users/${t.id}`)} className="bg-orange-500 hover:bg-orange-600 text-white text-xs font-medium px-4 py-1.5 rounded-md">Open Chat</button>
                  </td>
                </tr>
              ))}
              {filtered.length === 0 && (
