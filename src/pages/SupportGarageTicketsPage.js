    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg">
        <div className="px-5 pt-5">
          <h2 className="text-lg font-semibold text-gray-800">Garage Tickets</h2>
          <div className="mt-3 flex items-center gap-3">
            <input
              placeholder="Search Garage Tickets"
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="px-5 py-12 text-center">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">No garage tickets</h3>
            <p className="mt-1 text-sm text-gray-500">
              {query ? 'No tickets match your search criteria.' : 'There are currently no support tickets from garages.'}
            </p>
          </div>
        )}
                ...
                {filtered.map(t => (
                  <tr key={t.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-3 text-right">
                      <button 
                        onClick={() => navigate(`/support/garages/${t.id}`)} 
